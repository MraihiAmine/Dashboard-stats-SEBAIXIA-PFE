...
public class ProjectLineService {
    ...
    /**
     * Returns the top N most expensive cpCpe in resume.details across all ProjectLines.
     */
    public List<Map.Entry<String, Double>> getTopNExpensiveCpCpeInResume(int n) {
        List<ProjectLine> all = getAllProjectLines();
        Map<String, Double> cpCpeToMaxPrice = new HashMap<>();
        for (ProjectLine pl : all) {
            Object resumeObj = pl.getResume();
            if (resumeObj instanceof Map) {
                Object detailsObj = ((Map<?, ?>) resumeObj).get("details");
                if (detailsObj instanceof List) {
                    for (Object detailObj : (List<?>) detailsObj) {
                        if (detailObj instanceof Map) {
                            String cpCpe = (String) ((Map<?, ?>) detailObj).get("cpCpe");
                            Double totalPrice = null;
                            Object priceObj = ((Map<?, ?>) detailObj).get("totalPrice");
                            if (priceObj instanceof Number) {
                                totalPrice = ((Number) priceObj).doubleValue();
                            }
                            if (cpCpe != null && totalPrice != null) {
                                cpCpeToMaxPrice.merge(cpCpe, totalPrice, Math::max);
                            }
                        }
                    }
                }
        ...
        return cpCpeToMaxPrice.entrySet().stream()
    ...
    /**
     * Returns the top N most frequent cpCpe in resume.details across all ProjectLines.
     */
    public List<Map.Entry<String, Long>> getTopNFrequentCpCpeInResume(int n) {
        List<ProjectLine> all = getAllProjectLines();
        Map<String, Long> cpCpeToCount = new HashMap<>();
        for (ProjectLine pl : all) {
            Object resumeObj = pl.getResume();
            if (resumeObj instanceof Map) {
                Object detailsObj = ((Map<?, ?>) resumeObj).get("details");
                if (detailsObj instanceof List) {
                    for (Object detailObj : (List<?>) detailsObj) {
                        if (detailObj instanceof Map) {
                            String cpCpe = (String) ((Map<?, ?>) detailObj).get("cpCpe");
                            if (cpCpe != null) {
                                cpCpeToCount.merge(cpCpe, 1L, Long::sum);
                            }
                        }
                    }
                }
        ...
        return cpCpeToCount.entrySet().stream()
